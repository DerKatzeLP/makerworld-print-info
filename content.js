...
async function loadDebugSetting() {
  try {
    ...
    DEBUG.enabled = result.debugMode !== undefined ? result.debugMode : false
...
// Lade Einstellungen aus Storage (showPrintTime, showWeight, unit)
async function loadUserSettings() {
  try {
    ...
    return {
      showPrintTime:
        result.showPrintTime !== undefined ? result.showPrintTime : true,
      showWeight: result.showWeight !== undefined ? result.showWeight : true,
      unit: result.unit || "g",
      language: result.language || "en",
    }
  } catch (error) {
    log.error("Fehler beim Laden der User-Einstellungen:", error)
    return { showPrintTime: true, showWeight: true, unit: "g", language: "en" }
  }
}
...
// Funktion zum Formatieren des Gewichts (in Gramm oder oz)
...
async function fetchModelData(id, slug) {
  try {
    // Lade User-Einstellungen für Sprache
...
async function addPrintInfo(designCard, printTime, weight) {
  // Prüfen, ob bereits Info hinzugefügt wurde
  if (designCard.querySelector(".makerworld-print-info")) {
    return
  }

  // Finde die neue Einfügeposition: nach .design-bottom-row oder am Ende der Card
  let insertAfterElem = designCard.querySelector(".design-bottom-row")
  if (!insertAfterElem) {
    // Fallback: ans Ende der Card
    insertAfterElem = designCard.lastElementChild
    if (!insertAfterElem) {
      log.warn("Kein passendes Element zum Einfügen gefunden")
      return
    }
  }
  ...
  if (settings.showWeight) {
    html += `
            <div class="mw-css-1rkzspq" title="Gewicht">
                ...
                <span>${formatWeight(weight, settings.unit)}</span>
            </div>
        `
  }
  printInfoDiv.innerHTML = html

  // Füge die Info nach dem passenden Element ein
  if (insertAfterElem.nextSibling) {
    insertAfterElem.parentNode.insertBefore(
      printInfoDiv,
      insertAfterElem.nextSibling
    )
  } else {
    insertAfterElem.parentNode.appendChild(printInfoDiv)
  }
}
...
async function processDesignCard(designCard) {
  ...
  await addPrintInfo(designCard, printTime, weight)
}
...
browserAPI.storage.onChanged.addListener((changes, namespace) => {
  if (namespace === "sync" && changes.debugMode) {
    ...
    log.info(
      "Debug-Modus geändert:",
      DEBUG.enabled ? "aktiviert" : "deaktiviert"
    )
  }
})
